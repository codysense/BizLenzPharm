...
import { X, User, Play, Trash2 } from "lucide-react";
...
interface DetailPOSSaleModalProps {
  sale: any;
  onClose: () => void;
  isPending?: boolean; // Set true if viewing a paused sale
  onResume?: (sale: any) => void; // Callback to resume paused sale
  onDelete?: (id: string) => void; // Callback to delete paused sale
}
...
const DetailPOSSaleModal: React.FC<DetailPOSSaleModalProps> = ({
  ...
  // Query for payments only if it is a completed/real sale
  const { data: paymentsData } = useQuery({
    queryKey: ["posSalePayments", sale.id],
    queryFn: () => posApi.getPOSsalePayments(sale.id),
    enabled: !isPending && !!sale.id,
  });

  if (!sale) return null;

  // Normalize data between completed sales and pending sales
  const saleLines = isPending ? sale.pendingSaleLines : sale.saleLines;
  const displayPayments = isPending ? sale.payments : paymentsData;
  ...
  const discountAmount =
    saleLines?.reduce((sum: number, line: any) => {
          ...
          <div className="sticky top-0 z-20 bg-white border-b border-gray-200 px-6 py-5 flex items-center justify-between">
            <div>
              <p className="text-emerald-600 text-sm font-medium">
                {isPending ? "Pending POS Sale Details" : "POS Sale Details"}
              </p>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6">
              ...
              {saleLines?.length ? (
                <div className="space-y-4">
                  {saleLines.map((line: any, index: number) => {
                    const lineTotal =
                      Number(line.qty || 0) * Number(line.unitPrice || 0);
                    const discount =
                      (lineTotal * Number(line.discountPercent || 0)) / 100;
              ...
              <div className="flex gap-2">
                {isPending && onDelete && (
                  <button
                    ...
                    className="px-6 py-3 rounded-xl bg-red-100 hover:bg-red-500 hover:text-white text-red-600 transition flex items-center gap-2"
                  >
                ...
                {isPending && onResume && (
                  <button
                    ...
                    className="px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition flex items-center gap-2"
                  >
